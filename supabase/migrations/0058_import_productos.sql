-- 0058_import_productos.sql — Alta masiva de productos por Excel.
--
-- p_rows = [{ producto, descripcion, categoria_principal, categoria, temporada,
--             tela, talle, color, sku, precio_mayorista, stock, fila, estante, cubiculo }]
-- Una fila por variante; se agrupan por 'producto' (nombre). Auto-crea los
-- catálogos que falten (categoría principal/secundaria, temporada, tela, talle, color).
-- El stock inicial va al depósito p_warehouse. El precio Mayorista deriva el Público.
-- Atómico: si un SKU está repetido, aborta todo (la vista previa lo avisa antes).
...
begin
  ...
  loop
    ...
    -- Tipo de variación según lo que traigan las variantes.
    ...
    loop
      ...
      -- Catálogos de talle/color (para los desplegables del alta).
      if v_talle is not null then
        insert into public.sizes (organization_id, name) values (v_org, v_talle) on conflict (organization_id, name) do nothing;
      end if;
      if v_color is not null then
        insert into public.colors (organization_id, name) values (v_org, v_color) on conflict (organization_id, name) do nothing;
      end if;

      v_fila := nullif(v_el->>'fila', '')::int;
      v_est  := nullif(v_el->>'estante', '')::int;
      v_cub  := nullif(v_el->>'cubiculo', '')::int;
      ...
      v_stock := coalesce(nullif(v_el->>'stock', '')::int, 0);
      if v_stock <> 0 then
    ...
    -- Precio Mayorista (primer valor del producto) → deriva Público ×2.
    select nullif(e->>'precio_mayorista', '')::numeric into v_price
    from jsonb_array_elements(p_rows) e
    where trim(e->>'producto') = v_pname and nullif(e->>'precio_mayorista', '') is not null
    limit 1;

...
export default async function ProductoDetallePage({
  ...
  const { data: product } = await sb
    .from("products")
    .select(
      "id, name, description, variation_type, tax_rate, active, lifecycle, featured, portal_visible, tn_sync, external_id, categories(name), main_categories(name), seasons(name), fabric_types(name), product_variants(id, size, color, sku, barcode, active, loc_fila, loc_estante, loc_cubiculo)"
    )
    .eq("id", id)
    .single();
      ...
      <div className="mb-5 rounded-xl border border-line bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            {editable && <PortalToggle productId={product.id} visible={product.portal_visible !== false} />}

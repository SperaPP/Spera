...
const EXPORT_COLS = ["producto", "descripcion", "categoria_principal", "categoria", "temporada", "tela", "tipo", "iva", "activo", "tiene_foto", "destacado", "portal", "estado", "talle", "color", "sku", "codigo_barras", "variante_activa", "fila", "estante", "cubiculo", "precio_mayorista", "precio_publico", "promo_mayorista", "promo_publico", "stock"] as const;
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-line bg-card p-5">
        ...
        <p className="mb-3 text-sm text-muted">Elegí el <span className="font-medium text-ink">depósito</span>, exportá <span className="font-medium text-ink">todos los datos</span> (la columna <span className="font-medium text-ink">stock</span> es la de ese depósito), editá en el Excel y subilo. Se actualiza cada producto/variante matcheando por <span className="font-medium text-ink">SKU</span>, incluido el <span className="font-medium text-ink">stock del depósito elegido</span>, la publicación en el portal (columna <span className="font-medium text-ink">portal</span>, Sí/No) y los <span className="font-medium text-ink">precios promocionales</span> (columnas <span className="font-medium text-ink">promo_mayorista</span> / <span className="font-medium text-ink">promo_publico</span>; un número la activa, un <span className="font-medium text-ink">0</span> la quita). <span className="font-medium text-ink">Celda vacía = no cambia</span>. No toca la foto (se sube a mano).</p>
        <div className="flex flex-wrap items-center gap-2">
          <select value={whId} onChange={(e) => setWhId(e.target.value)} className="rounded-lg border border-line-strong bg-card px-2.5 py-1.5 text-sm text-ink outline-none focus:border-accent">
            {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
          </select>

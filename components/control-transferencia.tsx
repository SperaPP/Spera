...
import { recibirTransferencia, cancelarTransferencia } from "@/app/(app)/transferencias/actions";
...
export function ControlTransferencia({ transferId, status, items }: { transferId: string; status: string; items: Item[] }) {
  ...
  function recibir() {
    if (!complete) return toast.error("Faltan escanear prendas para aceptar la recepción.");
    start(async () => {
      const r = await recibirTransferencia(transferId);
  ...
  function cancelar() {
    if (!confirm("Cancelar la transferencia repone el stock en el origen. ¿Seguir?")) return;
    start(async () => {
      const r = await cancelarTransferencia(transferId);
      if (r.error) { toast.error(r.error); return; }
      toast.success("Transferencia cancelada. Stock repuesto en el origen.");
      router.refresh();
    });
  }
  ...
  return (
    <div className={card}>
      <div className="mb-3 flex items-center justify-between">
        <h2 className="text-sm font-medium text-ink">Recepción por escaneo</h2>
      ...
      <div className="relative mb-3">
        ...
        <input autoFocus className={`${input} h-12 pl-11`} placeholder="Escaneá cada prenda recibida…"
      ...
      {complete && (
        <div className="mt-4 flex flex-wrap items-center gap-3 rounded-xl border border-ok/30 bg-ok-bg px-4 py-3">
          ...
          <button onClick={recibir} disabled={pending} className="ml-auto rounded-lg bg-accent px-3.5 py-2 text-sm font-medium text-accent-fg hover:bg-accent-hover disabled:opacity-60">
            {pending ? "Recibiendo…" : "Aceptar y recibir"}
          </button>
        </div>
      )}

      <div className="mt-4 flex items-center justify-between border-t border-line pt-4">
        <p className="text-xs text-muted">No se acepta recepción parcial. Si algo no llegó, cancelá o resolvelo manualmente.</p>
